...
> Playwright, Chromium, MySQL과 Ollama가 사용자 PC의 Docker 환경에서 실행됩니다.
...
Runtime, Agent, MCP, Chromium과 필요한 라이브러리를 제공합니다. Compose가 Ollama를 실행하고
`qwen3:0.6b` 모델도 최초 실행 때 자동으로 내려받습니다. 웹 화면과 오디오는
...
이미지 빌드와 MySQL·Ollama·E-Class Quest 컨테이너 시작이 끝나면 웹 브라우저에서 다음 주소를 엽니다.
최초 실행은 `qwen3:0.6b` 모델을 받으므로 네트워크 환경에 따라 시간이 더 걸릴 수 있습니다.
...
```bash
docker compose --profile desktop stop desktop mysql ollama
docker compose --profile desktop start mysql ollama desktop
```
...
다른 MySQL·외부 Ollama·LMS 주소를 사용하려는 경우에만 [.env.example](./.env.example)을 복사해
필요한 항목을 변경합니다.
...
docker-compose.yml      Desktop 앱·MySQL·Ollama와 모델 준비 정의
...
- TUI·Agent·MCP·Playwright·Chromium·MySQL·Ollama는 모두 Docker에서 실행합니다.
- `qwen3:0.6b` 모델은 `ollama_data` 볼륨에 저장되어 컨테이너 재생성 후에도 유지됩니다.
