...
> 중앙 서버에 계정 정보를 보내는 서비스가 아닙니다. TUI, Agent, Playwright는 사용자 PC에서
> 실행되고 MySQL만 Docker 컨테이너로 실행합니다.
...
flowchart LR
    ...
    Playwright --> LMS[Hansung E-Class]
...
- Python 3.10 이상
- Docker Desktop과 WSL Integration(WSL) 또는 Docker Engine(Linux)
- Ollama와 `qwen3:0.6b`
- OpenAI API 키

Docker Desktop에서는 `Settings → Resources → WSL Integration`에서 사용하는 WSL 배포판을
활성화해야 합니다.
...
Docker Desktop을 실행한 뒤 다음 명령 하나만 입력합니다.

```bash
./run.sh
```

`run.sh`가 다음 작업을 자동으로 수행합니다.

1. `.venv` 확인
2. Docker Desktop 연결 확인
3. 개발용 MySQL 8.0 컨테이너 시작
4. MySQL healthcheck 대기
...
설정은 한 번만 저장됩니다. 모델이나 계정을 바꾸려면 다음 명령을 사용합니다.
...
일반 사용자는 `.env` 파일을 만들 필요가 없습니다. `run.sh`가 기본 Docker MySQL 주소를 자동으로
사용하고 나머지 값은 코드 기본값을 사용합니다.
...
run.sh                  MySQL 준비부터 TUI까지 실행하는 로컬 launcher
```
...
Windows에서 Docker Desktop을 실행하고 WSL Integration을 활성화합니다. 그래도 연결되지 않으면
PowerShell에서 다음 명령을 실행한 뒤 WSL을 다시 엽니다.

```powershell
wsl --shutdown
```

WSL에서 `docker version`을 실행했을 때 Client와 Server 정보가 모두 나와야 합니다.
