...
이 문서는 현재 `python -m app.main` 또는 `./run.sh`를 실행했을 때의 코드 흐름과, 반영된 3-Agent
...
| 파일 | 역할 |
|---|---|
| `run.sh` | Docker MySQL 시작·상태 확인·migration 후 `.venv`의 TUI 실행 |
...
- `#!/usr/bin/env bash`: 이 파일을 Bash로 실행한다.
- `set -e`: 명령이 실패하면 그대로 계속 진행하지 않는다.
- `set -u`: 정의하지 않은 변수를 사용하면 오류로 처리한다.
- `set -o pipefail`: 파이프 중간 명령이 실패해도 전체 실패로 처리한다.
- `BASH_SOURCE[0]`: 현재 `run.sh` 파일의 위치다.
- `cd "$PROJECT_ROOT"`: 어느 폴더에서 실행해도 `.env`, `data`, MCP 작업 폴더 기준을 프로젝트
  루트로 통일한다.
- 기본 MySQL URL이면 Docker Desktop의 MySQL 컨테이너를 자동 시작하고 health 상태를 기다린다.
- 다른 `MYSQL_URL`을 지정한 고급 사용자는 로컬 MySQL 컨테이너 시작을 건너뛴다.
- Alembic migration으로 DB 테이블을 현재 코드와 맞춘다.
- `exec`: Bash 프로세스를 Python 프로세스로 교체한다.
- `.venv/bin/python -m app.main "$@"`: 가상환경 Python으로 앱을 실행하고 `--setup` 같은 인자를 전달한다.

`cd app && python main.py`를 사용하면 프로젝트 루트가 Python import 경로에서 빠져
`ModuleNotFoundError: No module named 'app'`가 날 수 있다. 프로젝트 루트에서 `./run.sh` 또는
`./run.sh`를 사용한다.
